...
QBCore.Commands.Add('reportr', Lang:t("commands.reply_to_report"), {{name='id', help='Player'}, {name = 'message', help = 'Message to respond with'}}, false, function(source, args, rawCommand)
    ...
    if not QBCore.Functions.HasPermission(src, 'mod') then return end
...
QBCore.Commands.Add('setammo', Lang:t("commands.ammo_amount_set"), {{name='amount', help='Amount of bullets, for example: 20'}, {name='weapon', help='Name of the weapen, for example: WEAPON_VINTAGEPISTOL'}}, false, function(source, args)
    ...
    TriggerEvent('qb-log:server:CreateLog', 'staff', 'Staff Kill', 'red', string.format('%s has set ammo to %s with weapon %s', GetPlayerName(src), amount, weapon or "current weapon"), false)
    if weapon ~= nil then
...
QBCore.Commands.Add('bring', 'Bring a player to you (Mod only)', { { name = 'id', help = 'Player ID' }, }, true, function(source, args)
    if args[1] then
        local src = source
...
QBCore.Commands.Add('bringback', 'Bring back a player (Mod only)', { { name = 'id', help = 'Player ID' }, }, true, function(source, args)
    if args[1] then
        local src = source
