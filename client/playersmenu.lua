    ...
    local GiveItemMenuButton3 = GiveItemMenu:AddButton({
        ...
        select = function(btn)
            if itemname ~= 'Unknown' and itemamount ~= 0 then
                TriggerServerEvent('qb-admin:server:logAction', 'Give Item', string.format('%s gave %s (%s)', player.id, itemname, itemamount))
    ...
    local GiveMoneyMenuButton3 = GiveMoneyMenu:AddButton({
        ...
        select = function(btn)
            if moneytype ~= 'Unknown' and moneyamount ~= 0 then
                ...
                TriggerServerEvent('qb-admin:server:logAction', 'Give Money', string.format('%s gave %s (%s)', player.id, moneyamount, moneytype))
                moneytype = 'Unknown'
                moneyamount = 0
            else
                QBCore.Functions.Notify(Lang:t("error.give_item"), 'error')
    ...
    local SetMoneyMenuButton3 = SetMoneyMenu:AddButton({
        ...
        select = function(btn)
            if moneytype ~= 'Unknown' and moneyamount ~= 0 then
                ...
                TriggerServerEvent('qb-admin:server:logAction', 'Set Money', string.format('%s set %s (%s)', player.id, moneyamount, moneytype))
...
local function ChangePlayerName(adminId) 
    local admin = adminId
    local dialog = exports['qb-input']:ShowInput({
        header = 'Name Change',
    ...
    if dialog ~= nil then
        ...
        TriggerServerEvent('qb-admin:readyname', newfirstname , newlastname, admin)
    end
end
    ...
    PlayersButton2:On('select', function(item)
        ...
        local elements = {
            ...
            [4] = {
                ...
                value = "name",
                description = 'Change ' .. " " .. PlayerDetails.name .. " Name"
            }
        }
        for k, v in ipairs(elements) do
            local PlayerAdminButton = PlayerAdminMenu:AddButton({
                ...
                select = function(btn)
                    ...
                    elseif values == 'name' then
                        ChangePlayerName(PlayerDetails.id)
                        print(PlayerDetails.id)
            ...
            local PlayerExtraButton = PlayerExtraMenu:AddButton({
                ...
                select = function(btn)
                    ...
                    if values == 'inventory' then
                        TriggerEvent('qb-admin:client:inventory', PlayerDetails.id, source)
